    ...
    >
      <div className="flex max-h-full w-full max-w-2xl flex-col gap-3 overflow-auto rounded-xl border border-havoc-border bg-havoc-panel p-5 text-xs text-havoc-text">
        <div className="flex items-baseline justify-between gap-2">
          <h2 className="m-0 text-sm font-bold text-havoc-text">Report a bug</h2>
        ...
        <p className="m-0 text-[11px] leading-snug text-havoc-muted">
          Reporting is opt-in and anonymous. Nothing is sent automatically and there is no server —
          the buttons below open a pre-filled draft that you send yourself.
        </p>

        {ctx?.pendingCrash && (
          <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 px-2.5 py-2">
            <p className="m-0 text-[11px] text-amber-500">
              Freally Player closed unexpectedly last time. The crash report below is saved on this
              machine only.
            </p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-1">
          ...
          <pre className="m-0 max-h-48 overflow-auto rounded-md border border-havoc-border bg-havoc-surface px-2 py-1.5 font-mono text-[10px] leading-snug break-words whitespace-pre-wrap text-havoc-muted">
            {preview}
          </pre>
        </div>
